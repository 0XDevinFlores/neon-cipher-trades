...
import { Wallet, TrendingDown, Shield, BarChart3, Calculator, Lock, Loader2 } from "lucide-react";
...
const Put = () => {
  ...
  const [premium, setPremium] = useState(185.25);
  ...
  useEffect(() => {
    if (formData.strike && formData.size) {
      ...
      const timeValue = Math.max(0, (currentPrice * 0.08) - intrinsicValue);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const tx = await createPosition.writeAsync({
        args: [strikePrice, quantity, premiumAmount, false, expiryTime],
        value: BigInt(Math.round(premium * 1e18)), // Convert to wei
      });

      toast.success('PUT option created successfully!');
      console.log('Transaction hash:', tx);
    } catch (error) {
      console.error('Error creating PUT option:', error);
      toast.error('Failed to create PUT option');
    } finally {
      setIsLoading(false);
    }
  };
              ...
              <div className="space-y-4">
                ...
                <div className="p-4 bg-neon-red/10 border border-neon-red/30 rounded">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-lg font-bold text-neon-red">${premium.toFixed(2)}</span>
                  </div>
                ...
                <Button 
                  ...
                  className="w-full bg-neon-red hover:bg-neon-red/90 text-white font-bold py-6 text-lg disabled:opacity-50"
                >
                  {isLoading ? (
                    <>
